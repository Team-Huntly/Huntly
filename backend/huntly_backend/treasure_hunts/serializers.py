...
class MemoryThreadListSerializer(serializers.ModelSerializer):
    ...
    def get_memories(self, instance):
        memories = Memory.objects.filter(treasure_hunt=instance, clicked_by__in=Team.objects.filter(treasure_hunt=instance, team_members__id=self.context.get('request').user.id).first().team_members.all())
        memories = MemorySerializer(memories, many=True).data
        return memories
